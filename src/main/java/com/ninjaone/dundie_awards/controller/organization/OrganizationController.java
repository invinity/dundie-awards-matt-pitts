...
public class OrganizationController {
        ...
        @ResponseBody
        @Operation(summary = "Get all employees in an organization", parameters = {@Parameter(
                        name = "id",
                        description = "The ID of the organization to retrieve Employees for",
        ...
        public ResponseEntity<List<Employee>> getEmployees(@PathVariable("id") Long id) {
                return organizationRepository.findById(id)
                                .map(employeeRepository::findByOrganization).map(ResponseEntity::ok)
        ...
        @ResponseBody
        @Operation(summary = "Perform organization-wide employee actions", parameters = {@Parameter(
                        name = "id",
                        description = "The ID of the organization to perform the action on",
                        in = ParameterIn.PATH, required = true)},
                        requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                                        content = {@Content(schema = @Schema(
                                                        implementation = GiveDundieAwardsAction.class),
                        ...
                        responses = {@ApiResponse(responseCode = "200",
                                        ...
                                        content = {@Content(schema = @Schema(
                                                        implementation = GiveDundieAwardsResult.class))}),
        ...
        public ResponseEntity<OrganizationEmployeeActionResult> performEmployeeAction(
                        @PathVariable("id") Long id, @RequestBody OrganizationEmployeeAction action) {
                return organizationRepository.findById(id)
        ...
        private ResponseEntity<OrganizationEmployeeActionResult> performOrganizationAction(
                ...
                if (action instanceof GiveDundieAwardsAction) {
                        ...
                        return ResponseEntity.ok(GiveDundieAwardsResult.builder()
                                        .employeeUpdateCount(updateCount).build());
                }
